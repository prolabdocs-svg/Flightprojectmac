...
class AudioService {
   ...
   *  Legacy entry point kept for callers without per-aircraft idle/redline rpm —
   *  prefer updateFlight() when full FlightAudioState is available. */
  updateEngine(rpmFrac: number, running: boolean): void {
    ...
    g.osc1.frequency.setTargetAtTime(baseFreq, t, 0.05);
    g.osc2.frequency.setTargetAtTime(baseFreq * 2.01, t, 0.05);
    g.filter.frequency.setTargetAtTime(400 + clamped * 2600, t, 0.08);
    const targetGain = running ? 0.05 + clamped * 0.18 : 0.0001;
    g.gain1.gain.setTargetAtTime(targetGain, t, 0.12);
    g.gain2.gain.setTargetAtTime(targetGain * 0.6, t, 0.12);
  }
